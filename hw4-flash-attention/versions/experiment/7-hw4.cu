...
template <int bc, int br, int bc_pad, int bd, int num_warps, int threads_per_warp>
__global__ void flash_attention_kernel(float *O, float *Q, float *K, float *V, float *L, int N_pad, int N_q, int N_kv, int d_pad, int d, float *shared_mem);
template <int bc, int br, int bc_pad, int bd, int num_warps, int threads_per_warp>
...
void flash_attention(Data *data) {
    ...
    for (int i = 0; i < num_streams; i++) {
        ...
        cudaMemcpyAsync(d_Q + i * bb * N * d, Q + i * bb * N * d, num_batches * N * d * sizeof(float), cudaMemcpyHostToDevice, 0);
        cudaMemcpyAsync(d_K + i * bb * N * d, K + i * bb * N * d, num_batches * N * d * sizeof(float), cudaMemcpyHostToDevice, 0);
        cudaMemcpyAsync(d_V + i * bb * N * d, V + i * bb * N * d, num_batches * N * d * sizeof(float), cudaMemcpyHostToDevice, 0);

        cuda_pad_buffer(d_Q_pad + i * bb * N_pad * d_pad, d_Q + i * bb * N * d, num_batches, N, d, num_batches, N_pad, d_pad, 0);
        cuda_pad_buffer(d_K_pad + i * bb * N_pad * d_pad, d_K + i * bb * N * d, num_batches, N, d, num_batches, N_pad, d_pad, 0);
        cuda_pad_buffer(d_V_pad + i * bb * N_pad * d_pad, d_V + i * bb * N * d, num_batches, N, d, num_batches, N_pad, d_pad, 0);

        // Kernel launch
        float* shared_mem = nullptr;
        dim3 grid((int)ceilf((float)N / br), num_batches);
        cudaMalloc(&shared_mem, smem_size * grid.x * grid.y);
        dim3 block(num_warps * threads_per_warp);
        flash_attention_kernel<bc, br, bc_pad, bd, num_warps, threads_per_warp><<<grid, block, 0, 0>>>(
            d_O_pad + i * bb * N_pad * d_pad,
            d_Q_pad + i * bb * N_pad * d_pad,
            d_K_pad + i * bb * N_pad * d_pad,
            d_V_pad + i * bb * N_pad * d_pad,
            d_L_pad + i * bb * N_pad,
            N_pad, N, N, d_pad, d, shared_mem);

        // Asynchronous memory copy back to host
        cuda_unpad_buffer(d_O + i * bb * N * d, d_O_pad + i * bb * N_pad * d_pad, num_batches, N, d, num_batches, N_pad, d_pad, 0);

        cudaMemcpyAsync(O + i * bb * N * d, d_O + i * bb * N * d, num_batches * N * d * sizeof(float), cudaMemcpyDeviceToHost, 0);
    }
...
__global__ void flash_attention_kernel(float *O, float *Q, float *K, float *V, float *L, int N_pad, int N_q, int N_kv, int d_pad, int d, float *shared_mem) {
    ...
    float *oi = shared_mem + (blockIdx.y * gridDim.x + blockIdx.x) * (
        br * bd +  // oi
        br * bd +  // qi
        bc_pad * bd +  // kj
        bc_pad * bd +  // vj
