...
template <int bc, int br, int cr>
__global__ void flash_attention_kernel(float *O, float *Q, float *K, float *V, float *L, float *M, int N_pad, int N, int d);
...
void flash_attention(Data *data) {
    ...
    // }
    cudaMemcpyAsync(d_Q, Q, B * N_pad * d * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpyAsync(d_K, K, B * N_pad * d * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemcpyAsync(d_V, V, B * N_pad * d * sizeof(float), cudaMemcpyHostToDevice);
    cudaMemsetAsync(d_O, 0, B * N_pad * d * sizeof(float));
    cudaMemsetAsync(d_L, 0, B * N_pad * sizeof(float));
    init_float_array<<<(int)ceil((float)(B * N_pad) / 256), 256>>>(d_M, (B * N_pad), FLOAT_MIN);

    // Kernel launch
    dim3 grid((int)ceilf((float)N_pad / (br * cr)), B);
    dim3 block(bc, br);
    ...
    //         N, d);
    // }
    flash_attention_kernel<bc, br, cr><<<grid, block, smem_size>>>(
        d_O,
        d_Q,
        d_K,
        d_V,
    ...
    // }
    cudaMemcpyAsync(O, d_O, B * N_pad * d * sizeof(float), cudaMemcpyDeviceToHost);
...
__global__ void flash_attention_kernel(float *O, float *Q, float *K, float *V, float *L, float *M, int N_pad, int N, int d) {
    ...
    float *o = O + blockIdx.y * N_pad * d + blockIdx.x * cr * br * d;  // (cr, br, d)
    float *q = Q + blockIdx.y * N_pad * d + blockIdx.x * cr * br * d;  // (cr, br, d)
    float *k = K + blockIdx.y * N_pad * d;                             // (N, d)
    float *v = V + blockIdx.y * N_pad * d;                             // (N, d)
    float *l = L + blockIdx.y * N_pad + blockIdx.x * cr * br;      // (cr, br)
    float *m = M + blockIdx.y * N_pad + blockIdx.x * cr * br;      // (cr, br)
