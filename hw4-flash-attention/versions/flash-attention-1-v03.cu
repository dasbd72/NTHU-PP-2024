...
void flash_attention(Data *data) {
    ...
    for (int i = 0; i < num_streams; i++) {
        ...
        cudaMemsetAsync(d_O + i * bb * N * d, 0, num_batches * N * d * sizeof(float), streams[i]);
        cudaMemsetAsync(d_L + i * bb * N, 0, num_batches * N * sizeof(float), streams[i]);
        cuda_init_array(d_M + i * bb * N, num_batches * N, FLOAT_MIN, streams[i]);
...
__global__ void flash_attention_kernel(float *O, float *Q, float *K, float *V, float *L, float *M, int N, int d) {
    ...
    float *m = M + blockIdx.y * N + blockIdx.x * cr * br;          // (cr, br)
